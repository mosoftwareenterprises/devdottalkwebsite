...
describe('floating-register-button component', () => {
  ...
  describe('renders button structure', () => {
    it('should include Luma checkout script', () => {
      const html = renderTemplate(templatePath, { eventId: '12345' });
  ...
  describe('passes event ID correctly', () => {
    it('should use provided event ID', () => {
      ...
      const html = renderTemplate(templatePath, { eventId });
      
      expect(html).toContain(`data-luma-event-id="${eventId}"`);
    });
    ...
    it('should handle undefined event ID gracefully', () => {
      const html = renderTemplate(templatePath, {}); // No eventId provided
  ...
  describe('component updates with different contexts', () => {
    it('should work with event IDs containing special characters', () => {
      const eventId = 'event-2026-03-12';
      const html = renderTemplate(templatePath, { eventId });
  ...
  describe('no unwanted side effects', () => {
    it('should render without errors when used multiple times', () => {
      ...
      const html1 = renderTemplate(templatePath, { eventId: eventId1 });
      const html2 = renderTemplate(templatePath, { eventId: eventId2 });
      
      expect(html1).toContain(`data-luma-event-id="${eventId1}"`);
      expect(html2).toContain(`data-luma-event-id="${eventId2}"`);
    });

    it('should not contain hardcoded event IDs', () => {
      const html = renderTemplate(templatePath, { eventId: 'test-id' });
